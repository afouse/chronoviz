                                      ...
                                      addSheen:(BOOL)shouldAddSheen
{
    ...
    NSBitmapImageRep* rep = [NSBitmapImageRep imageRepWithData:[sourceImage TIFFRepresentation]];
...
- (CGImageRef) cgImageWithSize:(NSSize)theSize
{
	...
	[self drawInRect:NSMakeRect(0,0, theSize.width, theSize.height) fromRect:NSZeroRect/*sentinel, means "the whole thing*/ operation:NSCompositeCopy fraction:1.0];
	[NSGraphicsContext restoreGraphicsState];
...
- (CGImageRef) cgImage
{
	...
	return [self cgImageWithSize:[self size]];
